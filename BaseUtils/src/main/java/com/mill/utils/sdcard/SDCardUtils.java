...
import com.mill.utils.hideapi.HideApiHelper;
...
public class SDCardUtils {
    ...
    public static int getSdcardCount(Context context) {
        return getMountedSdcardList(context).size();
    }

    public static List<Sdcard> getMountedSdcardList(Context context) {
        List<Sdcard> list = new ArrayList<Sdcard>();
        Map<String, Sdcard> map = getSdcardList(context);
        for (Sdcard sdcard : map.values()) {
            if (!list.contains(sdcard) && Environment.MEDIA_MOUNTED.equalsIgnoreCase(sdcard.state)) {
                list.add(sdcard);
            }
        }
        return list;
    ...
    public static Map<String, Sdcard> getSdcardList(Context context) {
        HashMap<String, Sdcard> sdcardMap = new HashMap<String, Sdcard>();
        try {
            if (getAllSdcard_1(context, sdcardMap)) {
                return sdcardMap;
            } else if (getAllSdcard_2(sdcardMap)) {
                return sdcardMap;
            } else {
                getAllSdcard_3(sdcardMap);
            }
        } catch (Throwable e) {
            if (DEBUG) {
                Log.e(TAG, "", e);
    ...
    private static boolean getAllSdcard_3(HashMap<String, Sdcard> sdcardMap) {
        try {

            File file = Environment.getExternalStorageDirectory();
            String path = file != null ? file.getPath() : null;
            if (path != null && !sdcardMap.containsKey(path)) {
                Sdcard sdcard = new Sdcard();
                sdcard.path = path;
                sdcard.state = Environment.getExternalStorageState();
                sdcard.isEmulated = isExternalStorageEmulated();
//				sdcard.isPrimary = true;
                sdcard.isRemovable = isExternalStorageRemovable();
                sdcardMap.put(path, sdcard);
            }

            List<Mntent.mntent> mntents = Mntent.queryAll();
            if (mntents != null && mntents.size() > 0) {
                Object service = ReflectUtils.stubAsInterface(ReflectUtils.CLASSNAME_IMOUNTSERVICE_STUB, HideApiHelper.ServiceManager.getService("mount"));
                Method getVolumeState = service.getClass().getMethod("getVolumeState", String.class);
                for (Mntent.mntent item : mntents) {
                    if ("vfat".equalsIgnoreCase(item.mnt_type) && !"/mnt/secure/asec".equalsIgnoreCase(item.mnt_dir)) {
                        if (!sdcardMap.containsKey(item.mnt_dir)) {
                            try {
                                Sdcard sdcard = new Sdcard();
                                sdcard.path = item.mnt_dir;
                                sdcard.state = (String) getVolumeState.invoke(service, item.mnt_dir);
                                sdcard.isEmulated = false;
//								sdcard.isPrimary = false;
                                sdcard.isRemovable = true;
                                sdcardMap.put(item.mnt_dir, sdcard);

                            } catch (Exception e) {
                                if (DEBUG) {
                                    Log.d(TAG, "failed to invoke, path = " + path, e);
                                }
                                continue;
                            }
                        }
    ...
    private static boolean getAllSdcard_2(HashMap<String, Sdcard> sdcardMap) {
        try {
            Object service = ReflectUtils.stubAsInterface(ReflectUtils.CLASSNAME_IMOUNTSERVICE_STUB, HideApiHelper.ServiceManager.getService("mount"));
            Method getVolumeList = service.getClass().getMethod("getVolumeList");

            Object[] volumes = (Object[]) getVolumeList.invoke(service);
            if (volumes != null && volumes.length > 0) {
                Class<? extends Object> StorageVolumeClass = volumes[0].getClass();
                Method getPath = StorageVolumeClass.getMethod("getPath");
                Method isEmulated = StorageVolumeClass.getMethod("isEmulated");
                ...
                for (Object storageVolume : volumes) {
                    ...
                    if (!sdcardMap.containsKey(path)) {
                        try {
                            Sdcard sdcard = new Sdcard();
                            sdcard.path = path;
                            sdcard.state = getStorageVolumeStats(service, storageVolume, sdcard.path);
                            sdcard.isEmulated = (Boolean) isEmulated.invoke(storageVolume);
//							sdcard.isPrimary = (Boolean) isPrimary.invoke(storageVolume);
                            sdcard.isRemovable = (Boolean) isRemovable.invoke(storageVolume);
                            sdcardMap.put(path, sdcard);
    ...
    private static boolean getAllSdcard_1(Context context, HashMap<String, Sdcard> sdcardMap) {
        try {
            if (context != null) {
                Object service = ReflectUtils.stubAsInterface(ReflectUtils.CLASSNAME_IMOUNTSERVICE_STUB, HideApiHelper.ServiceManager.getService("mount"));
                Object storageManager = context.getSystemService(Context.STORAGE_SERVICE);
                Class<? extends Object> StorageManagerClass = storageManager.getClass();
                Method getVolumeList = StorageManagerClass.getMethod("getVolumeList");
